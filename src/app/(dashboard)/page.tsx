...
export default function DashboardPage() {
  ...
  return (
    <PageTransition>
      <motion.div
        className="max-w-[1240px] mx-auto space-y-2.5"
          ...
          return (
            <motion.div variants={sectionVariants} className="bg-gradient-to-r from-autronis-accent/10 via-autronis-card to-autronis-card border border-autronis-accent/20 rounded-2xl p-4">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {kpis.deadlinesDezeWeek > 0 && (
                    <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-red-500/15 text-red-400 tabular-nums">{kpis.deadlinesDezeWeek} deadline{kpis.deadlinesDezeWeek > 1 ? "s" : ""}</span>
                  )}
                </div>
              </div>

              {/* Start volgende taak — hero action */}
              {nextTask && (
                <div className="flex items-center gap-3 bg-autronis-accent/10 border border-autronis-accent/20 rounded-xl px-4 py-3 mb-2.5">
                  ...
                  <button
                    onClick={() => handleTaakAfvinken(nextTask.id)}
                    className="px-3 py-1.5 bg-autronis-accent hover:bg-autronis-accent-hover text-autronis-bg rounded-lg text-xs font-semibold transition-colors shrink-0"
                  >
                    Afvinken
                  </button>
                </div>
              ...
              <div className="space-y-1">
                ...
                {mijnTaken.length > 4 && (
                  <Link href="/taken" className="flex items-center gap-1 text-xs text-autronis-accent font-medium px-3 pt-0.5 hover:text-autronis-accent-hover">
                    +{mijnTaken.length - 4} meer <ArrowRight className="w-3 h-3" />
                  </Link>
                )}
              </div>
          </motion.div>
