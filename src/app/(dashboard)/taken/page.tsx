              ...
              >
                <div className={cn("flex items-center gap-2.5 px-3 py-1.5 bg-autronis-bg/20 rounded-lg border-l-[2px] transition-colors hover:bg-autronis-bg/40 group", pc.borderColor)}>
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <a href={`vscode://vscode.openFolder?folderUri=${encodeURIComponent("file://" + (taak.projectMap || ""))}&forceNewWindow=true`} className="p-0.5 text-autronis-text-secondary hover:text-purple-400 transition-colors" title="Terminal"><Terminal className="w-3 h-3" /></a>
        ...
        <div className="flex flex-wrap items-center gap-2 p-5 pt-4 border-t border-autronis-border/50 mt-4">
          {taak.status !== "afgerond" && (
            <>
              ...
              <a href={`vscode://vscode.openFolder?folderUri=${encodeURIComponent("file://" + (taak.projectMap || ""))}&forceNewWindow=true`}
...
export default function TakenPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { addToast("Taak verwijderd", "succes"); queryClient.invalidateQueries({ queryKey: ["taken"] }); queryClient.invalidateQueries({ queryKey: ["deadline-events"] }); },
    onSettled: () => { setMutatingCount((c) => c - 1); },
  });
