...
export const klantUren = sqliteTable("klant_uren", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  omschrijving: text("omschrijving"), // Auto-generated by Claude
  bron: text("bron", { enum: ["claude-sessie", "handmatig"] }).default("claude-sessie"),
  sessieId: text("sessie_id"), // e.g. 'SEM-LP-003'
  aangemaaktOp: text("aangemaakt_op").default(sql`(datetime('now'))`),
}, (table) => ({
...
export const chatSessies = sqliteTable("chat_sessies", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  gebruiker: text("gebruiker").notNull(), // 'sem' or 'syb'
  contextTag: text("context_tag").notNull(), // 'LP', 'DASH', etc.
  nummer: integer("nummer").notNull(), // incrementing per (gebruiker, context_tag)
  chatId: text("chat_id").notNull(), // 'SEM-LP-003' (composed, unique)
  onderwerp: text("onderwerp"), // short description
  klantId: integer("klant_id").references(() => klanten.id),
  projectId: integer("project_id").references(() => projecten.id),
  gestartOp: text("gestart_op").default(sql`(datetime('now'))`),
}, (table) => ({
  uniqChatId: uniqueIndex("idx_chat_sessies_chat_id").on(table.chatId),
  uniqGebruikerTagNr: uniqueIndex("idx_chat_sessies_uniq").on(table.gebruiker, table.contextTag, table.nummer),
}));
...
// ============ MODULE 6: SALES ============
// Proposals — deck-based proposal system met slides, DemoMode en PDF generatie
export const proposals = sqliteTable("proposals", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  klantId: integer("klant_id").references(() => klanten.id),
  titel: text("titel").notNull(),
  status: text("status", { enum: ["concept", "verzonden", "bekeken", "ondertekend", "afgewezen"] }).default("concept"),
  secties: text("secties").default("[]"),
