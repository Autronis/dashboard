...
//
// Deep work definitie hier: dagelijks (totaalActief - afleiding), gecappped
// op totaalActief zodat deepWorkMinuten ≤ actieve tijd ALTIJD geldt.
// Die definitie matcht /api/screen-time/sessies/route.ts zodat de dag-view
// en de week/maand-view op hetzelfde neerkomen (som van dagen = periode).
// ────────────────────────────────────────────────────────────────────────────
...
export async function aggregeerPeriode(
  ...
  // Per-dag buckets
  const perDagMap = new Map<string, { totaal: number; productief: number; afleiding: number }>();
  ...
  for (const e of rijen) {
    ...
    const bucket = perDagMap.get(dag) ?? { totaal: 0, productief: 0, afleiding: 0 };
  ...
  // Cap per-dag op 16 uur (idle noise filter) — proportional reductie van productief en afleiding
  ...
  for (const [datum, bucket] of [...perDagMap.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    // Deep work per dag: totaal - afleiding, nooit meer dan totaal
    const dagDeepWorkSec = Math.max(0, dagTotaal - dagAfleiding);
